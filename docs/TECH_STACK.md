...
18번은 영속 작업 기록과 worker를, 19번은 시점별 시장 관측·사건·차트를 도입했다.
20번은 기존 Codex CLI 실행과 조사 버전·상황별 후속 검토를 연결했다.
...
| 금융 차트 | TradingView Lightweight Charts | 도입. 저장된 분봉·일봉·거래량과 관측 이력을 렌더링한다. |
| 긴 작업 | 별도 Python worker + PostgreSQL 작업 기록 | 도입. 저장 자료 검증·읽기 전용 수집 작업의 상태·취소·재시작 복구를 관리한다. 분석·백테스트 실행 연결은 후속 범위다. |
...
연결했고 18번은 Jobs·Worker를 도입했다. HTTP에서는 작업을 제출하고 별도 worker가
실행한다. 네트워크 수집은 worker에 명시적 옵션을 지정했을 때만 가능하다.
...
둔다. 18번의 HTTP와 `trading jobs` CLI는 같은 작업 큐를 사용한다. 기존 동기 CLI·MCP
호출은 그대로이며 이를 모두 작업 큐로 바꾼 상태는 아니다.
...
재판단할 수 있어야 한다. 기능 20은 새 저장 관측·근거와 제안 시각을 조건으로 후속 조사를
접수한다. 상세와 현재 조건 감지 범위는 [Codex 조사](INVESTIGATIONS.md)를 따른다.
...
구조를 18번에서 구현했다. worker 재시작 후 복구, 외부 요청 제한, 중복 접수와 결과 저장을
함께 검증한다. 실행 방법과 작업·결과 보존 범위는 [작업 실행](JOBS.md)을 따른다.

토스 조회 등 기존 동기 I/O와 긴 CPU 계산이 API의 다른 요청을 막지 않도록 실행 경계를
설계한다. `async` 선언이나 프레임워크 교체만으로 기존 계산이 빨라진다고 가정하지 않는다.
기존 동기 CLI도 유지한다. 작업 큐에 연결된 종류와 나머지 계산 명령의 실행 경계를 구분한다.
...
사용자는 이후 주문 연결까지 전송 비활성 상태로 개발하도록 승인했다. 23번 계좌·주문·체결
대조, 24번 주문 관리, 25번 AI 운용 연결과 복구, 26번 결과 귀속과 판단 방식 비교까지
확장한다. 범위와 완료 조건은 [승인된 후속 개발](DEVELOPMENT_ROADMAP.md)을 따른다.
