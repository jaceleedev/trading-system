...
현재 작업은 문서화이며, 아래 신규 프론트엔드·API·worker는 아직 구현하지 않았다.
...
| 영역 | 채택 구성 | 현재 상태와 역할 |
| --- | --- | --- |
| 웹 화면 | Svelte 5 + SvelteKit + TypeScript | 도입 예정. 컴포넌트·상태 갱신과 페이지·공통 레이아웃·라우팅을 구성한다. |
| UI 기반 | Tailwind CSS + shadcn-svelte | 도입 예정. 필요한 컴포넌트 코드를 가져와 투자 작업실의 디자인과 동작에 맞춘다. |
| 웹 API | FastAPI | 도입 예정. 기존 Python 서비스에 HTTP 요청을 연결하고 입력·응답·오류 계약을 제공한다. |
...
| API 계약 공유 | OpenAPI 기반 TypeScript 클라이언트 생성 | 도입 예정. Hey API를 우선 검토하고 API 스키마와 프론트 타입을 함께 검증한다. |
| 서버 데이터 상태 | TanStack Query의 Svelte 어댑터 | 도입 예정. 조회·로딩·오류·캐시 갱신을 관리한다. |
...
| 개발·로컬 운영 | mise + uv + Docker Compose | 기존 구성 유지. 프론트 빌드용 Node와 패키지 관리 도구는 구현 시 프로젝트 범위에서 고정한다. |

신규 패키지의 세부 버전은 구현 시 상호 호환성을 확인하고 잠금 파일로 고정한다.
개발 기기에 도구가 설치돼 있다는 사실을 프로젝트 도입 완료로 기록하지 않는다.
현재 Python 의존성은 `uv.lock`으로 관리하며 프론트 패키지와 잠금 파일은 아직 없다.
...
목표 호출 구조는 다음과 같다. 이 그림은 아직 실행 중인 배포 구성이 아니다.
...
1. SvelteKit·FastAPI 연결과 API 계약, 기존 공통 서비스의 재사용 경계를 만든다.
2. 계좌·종목·연구 기록의 타입, 금융 수치·관측 시각·오류 표시를 연결한다.
3. 긴 작업의 실행·상태 조회·중복 방지·취소·복구를 구현한다.
4. 차트와 투자 작업실을 완성하고 실제 브라우저 사용 흐름을 검증한다.
5. 실제 사용 조건의 지연·메모리·SQL을 측정하고 필요한 최적화를 수행한다.

현재 문서 브랜치에서는 코드·의존성·실행 환경을 변경하지 않는다.
