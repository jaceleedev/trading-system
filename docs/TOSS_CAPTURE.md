...
현재 구현은 시세 관련 응답의 출처 보존 단계이며, 검증된 투자 자료로 자동 변환하지 않는다.
...
한 프로세스에서 요청 시작 간격은 최소 1.1초다. 여러 프로세스의 한도를 합산하지 않으므로
동시에 여러 수집기를 실행하지 않는다. 429는 자동 재시도하지 않고 종료한다.

수집 파일은 `provider`, `endpoint`, `query`, `retrieved_at`, `response`,
`contract_sha256`을 포함한다. `retrieved_at`은 **우리 시스템이 응답을 받은 시각**이다.
...
- `timestamp`는 봉 **시작** 시각이다. 응답에는 장 종료·최초 공개 시각·최종 확정 여부가 없다.
