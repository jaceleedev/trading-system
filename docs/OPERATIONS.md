...
| Python 직접 실행 화면 | 127.0.0.1:8501 | 연구 기록은 위 DB |
| Docker 화면 | 127.0.0.1:8502 | 같은 연구 DB |

두 화면은 같은 DB를 보는 대안이다. Docker 화면은 UID 10001, 읽기 전용 파일 시스템,
...
현재 실행되는 기본 자료는 합성 자료다. 토스의 시장 GET 원응답 수집은 구현했지만
실제 인증 요청은 실행하지 않았다. 실제 추천에 사용하려면 공급자의 과거 종목군,
