...
27번의 첫 화면 운영 요약은 API 응답, 작업 기능 설정, DB 접근, 같은 작업실의 worker
생존과 허용 기능을 따로 표시한다. 대기 이유와 조사 상태에서 해당 패널로 이동하고,
재검토 판단에서 저장 기록을 연다. 이는 29번의 전체 투자 단계 탐색과는 별도다.
정확한 worker 관측 의미와 상태 계약은 [JOBS.md](JOBS.md)를 따른다.

선택한 계좌 신선도와 판단 재검토 목록은 기존 문맥 계산을 재사용한다. 시장 자료는
한도 내 지원되는 최신 캡처의 관측 시각·조회 기준·경과 시간을 표시하며, 공통 신선도
기준은 미설정(`unknown`)이다. 최신 수집 시각이 최신 가격·확정 봉이나 전체 종목
범위를 의미하지 않는다. 미래 관측, 자료 없음, 미확인 숫자, 목록 생략을 구분한다.
시장 캡처의 `retrieved_at`과 봉의 원래 기준 시각은 그대로 별도 보존한다.

활성 작업·조사는 화면이 보이는 동안 5초 간격, 운영 상태만 남으면 30초 간격으로
최대 5분간 자동 재조회한다. 완료·일시정지·오류·숨겨진 탭은 불필요한 조회를 멈춘다.
명시적인 새로고침이나 다시 보이는 화면에서 관측을 다시 확인한다. 자동 조회는 저장
상태를 읽는 동작이며 새 계좌/시장 수집이나 투자 실행을 접수하지 않는다.
...
- 수집 완료 시각과 오래된 관측 여부를 표시한다. ‘저장 자료 다시 읽기’는 파일 재조회다.
  새 토스 관측은 [웹 수집 패널](OBSERVATION_CAPTURE.md)에서 명시적으로 접수한다.
  저장 관측이 없는 계좌의 첫 연결은 [계좌 CLI](TOSS_ACCOUNT.md)를 사용한다.
...
| `/api/v1/jobs/status` | 작업 기능 설정·DB 접근·workspace worker 독립 관측·대기 이유 |
| `/api/v1/market/catalog` | 저장 시장 캡처 목록·한도와 최신 지원 캡처의 관측 경과 시간 |
...
28번의 [웹 관측 수집](OBSERVATION_CAPTURE.md)은 저장 계좌 선택의 근거와 허용된
시장 조회 범위를 고정해 기존 작업을 접수한다. 응답 유실은 원래 request key와 입력으로
조회하고 완료 원본을 대조한 뒤 다음 조사·모의 입력에 명시적으로 연결한다.

29번의 [투자 단계 이어가기](GUIDED_INVESTMENT_FLOW.md)는 조사 버전과 출력부터
계획·대안·모의 원장·기간 보고서까지 선택을 주소에 보존한다. 서버의 읽기 전용
`GET /api/v1/guided-flow`가 실제 출처 연결을 대조하며 과거 관측과 현재 상단 계좌를
구분한다. 단계 이동은 조회와 입력 연결이다. 실행 요청은 별도 버튼으로 접수하고,
응답 유실 후에는 workspace별 브라우저 영수증의 원래 키와 입력으로 명시적으로
재확인한다. 연결 실패나 계좌 변경 후 늦은 응답을 현재 선택의 성공 결과로 표시하지 않는다.
