...
- 재검토 목록은 저장 기록에서 계산한다. 20번 AI 조사 패널은 별도로 허용한 worker에서
  저장된 시각·새 근거·종목 관측 조건을 점검해 후속 조사를 접수한다.
...
22번의 [전향적 모의 매매](PAPER_EXECUTION.md)는 명시한 초기 모의 현금과 선택 계좌의
관측 보유를 별도 원장에 둔다. 먼저 고정한 대안·가격 차이·거래량 참여율·수량 단위와
나중의 비수정 분봉을 연결한다. `--jobs`가 필요하며 실제 계좌나 자금 배정은 바꾸지 않는다.

23번의 [브로커 관측 대조](BROKER_RECONCILIATION.md)는 저장된 OPEN/CLOSED·개별 주문
관측과 이전·이후 계좌를 선택해 누적 값·비용·보유·매수 가능 금액의 변화를 구분한다.
조회·대조 계산은 오프라인이며 저장과 수집 작업 접수는 `--jobs`가 필요하다. 실제 GET은
`--allow-network` worker에서만 실행하고 합성 수집 작업은 인증 전에 거부한다.

24번의 [주문 의도 관리](ORDER_MANAGEMENT.md)는 선택한 계획·대안·활성 배정을 연결한다.
생성·정정·취소는 로컬 요청으로 보존하며 실제 전송 경로는 없다. `--jobs`가 필요하고
`--synthetic`에서만 고정된 합성 응답 검사를 제공한다. DB 전달 상태와 관측 상태는 따로
표시한다. 응답 유실·중단 복구는 미확정으로 남기며 자동 재전송·자금 해제를 하지 않는다.
...
26번의 [기간별 결과 비교](OUTCOME_ATTRIBUTION.md)는 선택한 모의 원장과 운용 흐름의
자료를 고정하고 결과를 저장한다. 모의 비용을 평가액에서 두 번 차감하지 않으며,
실제 브로커 자료의 별도 관측 기간·출처·미확인 현금 흐름을 보여 준다. 새 계산에는
`--jobs`가 필요하고 저장 보고서의 조회는 DB 없이 사용할 수 있다.
