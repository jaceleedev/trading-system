...
여전히 합성 자료이며 전략의 실제 수익성은 검증되지 않았다. 실제 주문 전송은 비활성이다.
...
[기간별 결과 비교](docs/OUTCOME_ATTRIBUTION.md)는 원장별·통화별 모의 평가 변화와 비용,
행동·판단 출처를 연결한다. 실제 브로커 관측의 기간·미확인 손익은 별도로 표시하며,
저장 결과는 DB 없이 원본 파일에서 다시 검증할 수 있다.
